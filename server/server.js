...
io.on('connection', function(socket) {
	...
	socket.on('play', function(data) {
		...
		var randomSpawnPoint = playerSpawnPoints[clients.length];
		currentPlayer = {
			name:data.name,
	...
	socket.on('player shoot', function(point) {
		console.log(currentPlayer.name+' recv: shoot' + point.position);
		var data = {
			...
			if(!data.isEnemy) {
				...
				enemies = enemies.map(function(enemy, index) {
					...
					if( a <= 50){
						...
						socket.emit('health', shoot);
						socket.broadcast.emit('health', shoot);

					//	console.log(enemies[index].name);
						//enemies.splice(index,1);
					 
						var posix =	Math.floor(Math.random() * 9)-9;
						var posiy =	Math.floor(Math.random() * 4)-4;
						var ro =	Math.floor(Math.random() * 180);
					...
					/*	 var enemiesResponse = {
						...
						// we always will send the enemies when the player joins
 						socket.emit('enemiesAdd', nenemy);
						 socket.broadcast.emit('enemiesAdd', nenemy);
